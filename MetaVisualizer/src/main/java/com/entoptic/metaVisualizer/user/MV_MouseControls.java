...
//		input = parent;
	 ...
	 */
	void handleMouseReleased(WMV_World world, MV_Display display, int mouseX, int mouseY, int frameCount)
	{
		mouseReleased = true;
...
//			display.map2D.handleMouseReleased(world, mouseX, mouseY);
	...
	void handleMousePressed(WMV_World world, MV_Display display, int mouseX, int mouseY, int frameCount)
	{
		if(display.getDisplayView() == 1)
