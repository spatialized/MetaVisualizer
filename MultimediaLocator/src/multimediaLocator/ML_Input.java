...
public class ML_Input
{
	...
	void handleKeyPressed(MultimediaLocator ml, char key, int keyCode)
	{
		...
		{
			/* General */
			handleUniversalKeyPressed(ml, key, keyCode);

			if(ml.display.displayView == 1)						 /* 2D Map View */
				handleMapViewKeyPressed(ml, key, keyCode);
			else if(ml.display.displayView == 2)				 /* Time View */
				handleTimelineViewKeyPressed(ml, key, keyCode);
			else if(ml.display.displayView == 3)				 /* Library View */
				handleLibraryViewKeyPressed(ml, key, keyCode);

			if (ml.state.interactive)							 /* World View Controls */
				handleInteractiveClusteringKeyPressed(ml, key, keyCode);
			else 						// Interactive Clustering Mode
			{
				handleGeneralKeyPressed(ml, key, keyCode);	 	 /* Controls for both 3D + HUD Views */

				if(!ml.display.inDisplayView())							
					handleWorldViewKeyPressed(ml, key, keyCode); /* Controls only for World View */
			}
		}
	}
...
//		}
	...
	void handleTimelineViewKeyPressed(MultimediaLocator ml, char key, int keyCode)
	{
		if (key == 'j') 
	...
	private void handleUniversalKeyPressed(MultimediaLocator ml, char key, int keyCode)
	{
		if (key == 'R')
			ml.restart();
...
//			}
	...
	private void handleGeneralKeyPressed(MultimediaLocator ml, char key, int keyCode)
	{
		if (key == '0')
		{
	...
	private void handleWorldViewKeyPressed(MultimediaLocator ml, char key, int keyCode)
	{
		if (key == 'j') 
...
//			else
	...
	private void handleMapViewKeyPressed(MultimediaLocator ml, char key, int keyCode)
	{
		if (key == 'a') 
...
//			ml.display.map2D.resetMapZoom(ml.world, true);
	...
	private void handleLibraryViewKeyPressed(MultimediaLocator ml, char key, int keyCode)
	{
		if (key == 'j') 
			ml.world.viewer.moveToRandomCluster(true, false);				/* Teleport to random cluster */
...
//					if(ml.world.getField(selectedField).hasBeenVisited())
	...
	private void handleInteractiveClusteringKeyPressed(MultimediaLocator ml, char key, int keyCode)
	{
		if (!optionKey && key == 'h')
		{
			if(!ml.world.getState().hierarchical)
