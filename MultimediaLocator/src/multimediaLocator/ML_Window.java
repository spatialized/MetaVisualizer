...
//	public GCheckbox chkbxMovementTeleport, chkbxFollowTeleport;
	...
	/* Library View Window */
	public GLabel lblViewerStats, lblWorldStats, lblInterestPoint;					
...
//		private int shortWindowHeight = 340, mediumWindowHeight = 600, tallWindowHeight = 880;
		...
		libraryViewWindowHeight = mediumWindowHeight + 25;
	...
	public void setupMainMenu()
	{
		...
		lblViewMode.setText("Current View");
		...
		x = 55;
		y += iLargeBoxHeight;
		optWorldView = new GOption(mainMenu, x, y, 100, iVerySmallBoxHeight, "World (1)");
		...
		optMapView = new GOption(mainMenu, x += 110, y, 80, iVerySmallBoxHeight, "Map (2)");
		...
		optLibraryView = new GOption(mainMenu, x += 110, y, 115, iVerySmallBoxHeight, "Library (4)");
		...
		y = mainMenuHeight - iBottomTextY * 5 / 2;
	...
	public void setupNavigationWindow()
	{
		...
		optTeleport = new GOption(navigationWindow, x+=90, y, 150, iVerySmallBoxHeight, "Teleport  (t)");
...
//		chkbxAngleThinning.setSelected(world.viewer.getSettings().angleThinning);
		...
		if(compressTallWindows) x += windowWidth;
		y += iMediumBoxHeight;
		btnViewSelected = new GButton(mediaWindow, x, y, 115, iVerySmallBoxHeight, "View (⏎)");
...
//		chkbxDomeView.setSeleted(world.viewer.getSettings().);
	...
	public void setupLibraryViewWindow()
	{
		...
		btnPreviousCluster = new GButton(libraryViewWindow, x+135, y, 100, iVerySmallBoxHeight, "Next (right)");
		...
		x = 0;
		y += iVeryLargeBoxHeight;
		lblViewerStats = new GLabel(libraryViewWindow, x, y, libraryViewWindow.width, iMediumBoxHeight, "Viewer");
		...
		lblShift3 = new GLabel(libraryViewWindow, x, y, libraryViewWindow.width, iVerySmallBoxHeight);						/* Display Mode Label */
		lblShift3.setText("Press SHIFT + 4 to show / hide");
...
//			applet.background(0);
			...
			float x = iLeftMargin;			// Starting horizontal position
			float y = libraryWindowViewerTextYOffset;					// Starting vertical position
			...
			if(world.viewer.getState().getCurrentClusterID() >= 0)
			{
				...
				y = libraryWindowWorldTextYOffset;
				if(world.getFieldCount() > 1)
