...
public class MV_Stitcher 
{
	...
	public MV_Stitcher(WMV_World parent)
	{
		p = parent;
	...
	public WMV_Panorama stitch(String library, List<Integer> imageList, int clusterID, int segmentID, List<Integer> selected)
	{
		...
		while(!success || end)
		{
			...
			if(!end)
			{
				...
				if(!imgs.isNull())
				{
					if(p.mv.debug.stitching) System.out.println("Attempting to stitch "+imgs.size()+" images...");
					...
					{
						if(p.mv.debug.stitching) p.mv.display.message(p.mv, "Error #" + status + " couldn't stitch panorama...");
				...
				{
					if(p.mv.debug.stitching) p.mv.display.message(p.mv, "Couldn't stitch panorama... No images!");
					break;
				}
			}
			else break;
		...
		if(success)				// Stitching succeeded
		{
			...
			if(p.mv.debug.stitching) p.mv.display.message(p.mv, "Panorama stitching successful, output to file: " + fileName);
		...
		if(success)
		{
			...
			if(p.mv.debug.stitching)	// TESTING
			{	
				String filePath = "";
				String fileName = "";
				...
				if(p.mv.debug.stitching) p.mv.display.message(p.mv, "Debugging: output panorama with borders to file: " + fileName);
...
//		org.bytedeco.javacpp.opencv_imgcodecs.imwrite(sketchPath("")+"/output.jpg", dst);
	...
	{
	  PGraphics pg = p.mv.createGraphics(img.width, img.height, PApplet.JAVA2D);
